...
##################################### API Used of Instalment Controller are given below###########################
   ...
   ## POST /payFullBalance/{orderId}-->Update instalment plan when customer wants to pay full outstanding balance.
   ## @Author: Sumeet Kumar

@ApiTest_UK
@ApiTest_DE
@ApiTest_US
Feature: Verify POST Pay full balance API (Change instalment plan)
...
#  @PayFullPaymentCaptured
  Scenario: To verify POST Pay full balance API after first payment captured
...
#    Then validate in response 200 should be there and message "The capture payment request has been sent successfully"
    ...
    Then response of pay full balance should be 200
    And verify success message of pay full balance "pay.full.response.message"
    Given authentication token and order id has been set and Get Instalment Plan api has been hit
    Then verify all instalments paid

#  @PayFullPaymentNotCaptured
  Scenario: To verify POST Pay full balance API when first payment not captured
    Then hit POST Pay full balance to update instalment plan
    Then response of pay full balance should be 404
    Then verify instalment plan not found message "instalment.plan.not.found"