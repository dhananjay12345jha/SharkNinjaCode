...
Feature: Login: Validate login using valid credentials
  ...
  Scenario Outline: Validate login using invalid credentials
    ...
    Examples:
      ...
      | valid   | invalid  | login.email      |              | Please enter a password.                                         | Veuillez entrer un mot de passe.                                            |
      | valid   | valid    | xyz1@yopmail.com | abc@1234     | Your E-mail/Password combination is incorrect. Please try again. | Votre combinaison courriel/mot de passe est incorrecte. Veuillez réessayer. |
