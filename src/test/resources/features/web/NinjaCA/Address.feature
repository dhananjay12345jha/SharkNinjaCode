...
@regressionNinjaCA
@regNinjaCASuit2
Feature: Edit Address: I want to edit address and verify if address is edited.
  ...
  Scenario: Verify address is successfully edited with correct details
    ...
    When I fill the form to add address with information given below
      ...
      | Sanket    | Jha      | 4 Queens Rd   | E4L 4G5    | Sackville | Alberta  | 9876543210 |
    ...
    Then Address should be edited with given below details
      ...
      | Sanket    | Jha      | 4 Queens Rd   | E4L 4G5    | Sackville | 9876543210 |
    When I click edit address link for editing a address
    Then edit an address form should be visible
    When I fill the form to add address with information given below
      | FirstName | LastName | StreetAddress | PostalCode | City      | Province | Phone      |
      | Sumeet    | Kumar    | 4 Queens Rd   | E4L 4G5    | Sackville | Alberta  | 9599108719 |
    And I click on save address button
    Then Address should be edited with given below details
      | FirstName | LastName | StreetAddress | PostalCode | City      | Phone      |
      | Sumeet    | Kumar    | 4 Queens Rd   | E4L 4G5    | Sackville | 9599108719 |
    And language selected should be equal to "select.language.to.test"
  ...
  Scenario: Validating error messages to be shown corresponding to all mandatory fields
    ...
    Then validate that for mandatory field "Last Name" or "Nom de famille" error message should be shown
    ...
    Then validate that for mandatory field "Postal Code" or "Code postal" error message should be shown
    Then validate that for mandatory field "Province" or "Province" error message should be shown
    And language selected should be equal to "select.language.to.test"
